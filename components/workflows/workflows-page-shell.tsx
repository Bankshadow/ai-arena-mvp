...
type WorkflowsPageShellProps = {
  workflows: WorkflowCard[];
  source: "database" | "mock";
};

export function WorkflowsPageShell({ workflows, source }: WorkflowsPageShellProps) {
  ...
  return (
    <div className="relative min-h-screen bg-[#030303] text-zinc-100">
      ...
      <main className="relative mx-auto max-w-6xl px-4 pb-20 pt-10 sm:px-6">
        <p className="font-mono text-xs uppercase tracking-[0.2em] text-cyan-400/80">{w.eyebrow}</p>
        <h1 className="mt-2 flex items-center gap-3 text-3xl font-semibold sm:text-4xl">
          <GitBranch className="size-8 text-violet-400" />
          {w.title}
        </h1>
        <p className="mt-3 max-w-2xl text-zinc-400">{w.description}</p>
